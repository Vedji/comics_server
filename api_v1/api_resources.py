...
def init_site_api(app: flask.app.Flask, get_db):
    ...
    def api_resources_add_work():
        ...
        if not api_v1.bd_connect.BDConnect.is_user_authorization(
                get_db(), request.form["login"], request.form["password"]):
            return {"status": "error", "code": 401, "message": "No authorization"}
        per = api_v1.bd_connect.BDConnect.get_user_permission_level(
            get_db(), request.form["login"], request.form["password"])
        if per > 2 or per < 1:
        ...
        if request.form["id"][0] == '_' or not api_v1.genre_template.Genre.is_english_alphabet(request.form["id"]):
            return {"status": "error", "code": 400, "message": "Id's is not on english and symbols '_' "}
        if api_v1.bd_connect.BDConnect.is_work_id_exists(get_db(), request.form["id"]):
    ...
    def api_resources_add_work_chapter():
        ...
        if not api_v1.bd_connect.BDConnect.is_work_id_exists(get_db(), request.form["id"]):
    ...
    def get_all_work_genres():
        return {"status": "success", "data": api_v1.genre_template.Genre.get_work_dict_genre()}
