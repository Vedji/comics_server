...
def init_site_api(app: flask.app.Flask, get_db):
    ...
    def api_bd_get():
        ...
        result = api_v1.bd_connect.BDConnect.get_bd_page(
    ...
    def api_user_reg():
        if "user_login" not in request.json or "user_password" not in request.json:
            return jsonify({"result": False})
        if not api_v1.bd_connect.BDConnect.is_user(get_db(), request.json["user_login"]) \
                and len(request.json["user_password"]) > 6:
            api_v1.bd_connect.BDConnect.user_registration(
                get_db(), request.json["user_login"], request.json["user_password"])
    ...
    def api_user_user_info():
        if "user_password" not in request.json or "user_login" not in request.json:
            return jsonify({"result": False})
        data = api_v1.bd_connect.BDConnect.api_get_user_info(
    ...
    def api_catalog_info():
        ...
        data = api_v1.bd_connect.BDConnect.get_info_manga(get_db(), manga_name)
        result = {"result": False}
        if data:
            result = data
            data["result"] = True
    ...
    def api_catalog_manga_info():
        manga_name = request.args.get("name")
        data = api_v1.bd_connect.BDConnect.get_list_manga_chapters(get_db(), manga_name)
        result = {"result": False}
        if data:
            result = data
            data["result"] = True
    ...
    def api_catalog_manga_chapter():
        ...
        try:
            ...
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.get_info_chapter(get_db(), manga_name, chapter)
        if data:
            result = data
            result["result"] = True
        return jsonify(result)
    ...
    def api_catalog_manga_page():
        ...
        try:
            ...
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.get_info_page(get_db(), manga_name, chapter_number, page_number)
        if data:
            result = data
            result["result"] = True
        return jsonify(result)
    ...
    def api_catalog_manga_comments():
        ...
        try:
            ...
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.api_get_manga_comments(get_db(), manga_name, limit, offset)
    ...
    def api_catalog_manga_is_read():
        ...
        if "user_password" not in request.json or "user_login" not in request.json or "manga_name" not in request.json:
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.manga_in_likes_list(
            get_db(),
    ...
    def api_catalog_manga_set_manga():
        ...
        if "user_password" not in request.json or "user_login" not in request.json or "manga_name" not in request.json:
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.add_manga_in_like_manga_user(
            get_db(),
            request.json["user_login"], request.json["user_password"], request.json["manga_name"])
    ...
    def api_user_del_comment():
        ...
        if "user_password" not in request.json or "user_login" not in request.json or "manga_name" not in request.json:
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.delete_user_comment(
            get_db(),
            request.json["user_login"], request.json["user_password"], request.json["manga_name"])
    ...
    def api_user_read_list_get():
        ...
        try:
            ...
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.get_user_read_catalog_list(
            get_db(),
    ...
    def api_user_comments_list_get():
        ...
        try:
            ...
            return jsonify(result)
        data = api_v1.bd_connect.BDConnect.get_user_comments(
            get_db(),
            user_login, user_password, limit, offset)
        if data and "comments" in data:
            result = data
    ...
    def api_manga_get_catalog():
        ...
        if "template" not in request.json:
            result = api_v1.bd_connect.BDConnect.get_catalog_manga(
    ...
    def upload_file():
        ...
        if file:
            file.save('uploads/' + file.filename)  # Path to directory, where saved manga images
