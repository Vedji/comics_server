...
def init_site_api(app: flask.app.Flask, get_db, jwt):
    ...
    def api_get_user_info():
        user_id = get_jwt()["ID"]
        return jsonify({"status": "success", "data": api_v1.bd_connect.BDConnect.is_user_exists(get_db(), user_id)})

    @app.route('/api/v1/user/login', methods=['POST'])
    def _api_user_login():
        username = request.json.get('username', None)
        password = request.json.get('password', None)
        data = api_v1.bd_connect.BDConnect.get_user_info(get_db(), username, password)
        if not data:
    ...
    def _api_user_registration():
        ...
        if api_v1.bd_connect.BDConnect.is_user(get_db(), username):
            return jsonify({"status": "error", "code": 400, "message": "User has exists"}), 400
        data = api_v1.bd_connect.BDConnect.user_reg(get_db(), username, password)
        expires = timedelta(days=1)
    ...
    def api_get_user_likes():
        ...
        data = api_v1.bd_connect.BDConnect.get_user_likes_catalog(get_db(), header_login["ID"], limit, offset)
    ...
    def api_get_user_like_work(work_id: str):
        header_login = get_jwt()
        data = api_v1.bd_connect.BDConnect.is_user_like_work(get_db(), header_login["ID"], work_id)
        if data is None:
    ...
    def api_set_user_like_work(work_id: str):
        header_login = get_jwt()
        data = api_v1.bd_connect.BDConnect.set_user_like_work(get_db(), header_login["ID"], work_id)
        if data is None:
            return jsonify({"status": "error", "code": 404, "message": "Work is not exists"}), 404
        return jsonify({"status": "success", "data": {"user_like": data}}), 200
    ...
    def api_get_user_comments_list():
        ...
        offset = request.args.get('offset', 0, int)
        header_login = get_jwt()
        data = api_v1.bd_connect.BDConnect.get_user_comment_list(get_db(), header_login["ID"], limit, offset)
        return {
            "status": "success",
            "data": data
        }
    ...
    def api_get_user_works_comment(work_id: str):
        header_login = get_jwt()
        try:
            data = api_v1.bd_connect.BDConnect.get_user_comment(get_db(), header_login["ID"], work_id)
            return jsonify({
                "status": "success",
                "data": data
            }), 200
        except api_v1.exception.NotExists as body:
    ...
    def api_del_user_works_comment(work_id: str):
        ...
        api_v1.bd_connect.BDConnect.del_works_comment(get_db(), comm_id, work_id)
    ...
    def api_add_user_works_comment(work_id: str):
        user_data = get_jwt()
        rating, msg, _ = api_v1.exception.get_arg(request.json["rating"], lambda x: int(x), "rating")
        if msg["status"] == "error":
            return jsonify(msg), msg["code"]
        comment, msg, _ = api_v1.exception.get_arg(request.json["comment"], lambda x: str(x), "comment")
        if msg["status"] == "error":
            return jsonify(msg), msg["code"]
        data, msg, _ = api_v1.exception.req_fun(
            lambda:
            api_v1.bd_connect.BDConnect.append_works_comment(get_db(), user_data["ID"], work_id, rating, comment))
        if msg["status"] == "error":
            return jsonify(msg), msg["code"]
        return jsonify({"status": "success", "data": {}}), 200
    ...
    def api_upd_user_works_comment(work_id: str):
        ...
        data, msg, _ = api_v1.exception.req_fun(
            lambda:
            api_v1.bd_connect.BDConnect.update_works_comment(get_db(), user_data["ID"], work_id, rating, comment))
        if msg["status"] == "error":
            return jsonify(msg), msg["code"]
        return jsonify({"status": "success", "data": {}}), 200
