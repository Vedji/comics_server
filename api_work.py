...
import bd_connect
from genre_template import Genre
import os
...
def init_site_api(app: flask.app.Flask, get_db):
    ...
    def api_add_work():
        ...
        if work_id == '_' or not genre_template.Genre.is_english_alphabet(work_id):
            return jsonify({"status": "error", "code": 400, "message": "Id's is not on english and symbols '_' "}), 400
        if bd_connect.BDConnect.is_work_id_exists(get_db(), work_id):
    ...
    def api_get_work(work_id: str):
        ...
        if type(work_id) is not str or len(work_id) <= 0:
            return jsonify({"status": "error", "code": 400, "message": "Argument work_id is not exists"}), 400
        data = bd_connect.BDConnect.get_work(get_db(), work_id)
        if data:
            data["genre_headers"] = Genre.get_manga_genre(data["genre"])
    ...
    def api_get_work_chapters(work_id: str):
        ...
        data = bd_connect.BDConnect.get_work_chapters(get_db(), work_id)
        if data is not None:
            return jsonify({
                "status": "success",
                "data": {
    ...
    def api_add_work_chapter_catalog(work_id: str):
        ...
        if not bd_connect.BDConnect.is_work_id_exists(get_db(), work_id):
        ...
        if bd_connect.BDConnect.work_chapter_exists(get_db(), work_id, chapter_num):
